...
Но только 3/274 QR-кода читаемы на GT-фреймах: изображение слишком
размытое или мелкое для стандартных и нейросетевых декодеров.

**Мультикадровый тест** (±450 мс вокруг GT-фрейма): ещё 2/20 ценников
читаются на соседних кадрах (+150 мс). QR-код находится в узком
«окне видимости» — ByteTrack должен его захватить.
...
- Multi-frame fallback: +1 WeChatQR вызов на трек (≤4.8 с), если основные кандидаты не дали QR

**Следующие шаги (приоритет ROI):**
1. **QR-специфичный SR** (EDSR/HAT на синтетике QR) — общий RealESRGAN галлюцинирует модули
2. **TrOCR fine-tune** на синтетических ценниках → product_name ≥40%
