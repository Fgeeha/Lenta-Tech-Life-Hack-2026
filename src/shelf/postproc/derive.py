...
def apply_field_derivation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "price4_qr" in result.columns and "price_card" in result.columns:
        mask = result["price4_qr"].apply(_is_empty) & ~result[
            "price_card"
        ].apply(_is_empty)
        if mask.any():
    ...
    if "price1_qr" in result.columns and "price_default" in result.columns:
        mask = result["price1_qr"].apply(_is_empty) & ~result[
            "price_default"
        ].apply(_is_empty)
        if mask.any():
    ...
    if "price_default" in result.columns and "price1_qr" in result.columns:
        mask = result["price_default"].apply(_is_empty) & ~result[
            "price1_qr"
        ].apply(_is_empty)
        if mask.any():
    ...
    if "price_card" in result.columns and "price4_qr" in result.columns:
        mask = result["price_card"].apply(_is_empty) & ~result[
            "price4_qr"
        ].apply(_is_empty)
        if mask.any():
    ...
    if "qr_code_barcode" in result.columns and "barcode" in result.columns:
        ...
        mask = result["qr_code_barcode"].apply(_is_empty) & ~result[
            "barcode"
        ].apply(_is_empty)
        for idx in result[mask].index:
            normalized = _normalize_barcode(
                str(result.at[idx, "barcode"]).strip(), strict=True
            )
            if normalized:
                result.at[idx, "qr_code_barcode"] = normalized
        # qr_code_barcode → barcode
        mask = result["barcode"].apply(_is_empty) & ~result[
            "qr_code_barcode"
        ].apply(_is_empty)
        for idx in result[mask].index:
            normalized = _normalize_barcode(
                str(result.at[idx, "qr_code_barcode"]).strip(), strict=True
            )
            if normalized:
    ...
    if "discount_amount" in result.columns:
        ...
        for idx in result[mask].index:
            pc = (
                _parse_price(result.at[idx, "price_card"])
                if "price_card" in result.columns
                else None
            )
            pd_val = (
                _parse_price(result.at[idx, "price_default"])
                if "price_default" in result.columns
                else None
            )
            if (
                pc is not None
                and pd_val is not None
                and pd_val > 0
                and pc < pd_val
            ):
                pct = math.floor((1.0 - pc / pd_val) * 100)
                result.at[idx, "discount_amount"] = (
                    f"-{pct}%" if 1 <= pct <= 99 else "нет"
                )
            else:
