...
def test_defaults_fill_empty():
    ...
    assert (
        pd.isna(out.loc[0, "additional_info"])
        or out.loc[0, "additional_info"] == ""
    )  # P1 field, not filled by P0
...
def test_derive_discount_amount_from_prices():
    ...
    out = apply_field_derivation(df)
    assert (
        out.loc[0, "discount_amount"] == "-30%"
    )  # floor((1-70/100)*100)=30, GT format is negative
...
def test_catalog_lookup_prefix_fallback_extra_digit():
    ...
    entry = cat.lookup(
        barcode="46904911225870"
    )  # catalog bc is substring of input
    assert entry is not None
    assert entry.product_name == "Widget A"
...
def test_lookup_by_video_price_dual_tiebreaker():
    ...
    entry = cat.lookup_by_video_price(
        "149,99", price_default="189,99", video_hint="49_5"
    )
    assert entry is not None
...
def test_lookup_by_price_and_name_resolves_collision():
    cat = _make_catalog(
        [
            {
                "barcode": "4690000000016",
                "price_card": "159,99",
                "product_name": "Молоко Простоквашино 3.2% 1л",
                "src": "49_5.csv",
            },
            {
                "barcode": "4690000000023",
                "price_card": "159,99",
                "product_name": "Кефир Домик в деревне 2.5% 1л",
                "src": "49_5.csv",
            },
            {
                "barcode": "4690000000030",
                "price_card": "159,99",
                "product_name": "Йогурт Активиа 290г",
                "src": "49_5.csv",
            },
        ]
    )
    ...
    assert (
        cat.lookup_by_video_price_and_name(
            price_card="159.99", video_hint="49_5"
        )
        is None
    )
    ...
    cat = _make_catalog(
        [
            ...
            },
            {
                "barcode": "4690000000023",
                "price_card": "159,99",
                "product_name": "Молоко Домик в деревне 3.2% 1л",
                "src": "49_5.csv",
            },
        ]
    )
    entry = cat.lookup_by_video_price_and_name(
        price_card="159.99", product_name_ocr="молоко 3 2", video_hint="49_5"
    )
    assert entry is None
...
def test_lookup_by_price_and_name_requires_video():
    ...
    assert (
        cat.lookup_by_video_price_and_name(
            price_card="159.99", product_name_ocr="Простоквашино", video_hint=""
        )
        is None
    )
...
def test_force_prices_overwrites_wrong_ocr_on_exact_barcode_match():
    ...
    tag = PriceTag(
        barcode="4690491122587", price_card="999,00", price_default="111,00"
    )
...
def test_dot_price_leaves_non_numeric_strings():
    ...
    assert (
        _dot_price("Молоко, 1л") == "Молоко, 1л"
    )  # product_name comma unchanged
...
def test_detection_area_is_int():
    ...
    det = Detection(
        x_min=10.3, y_min=20.7, x_max=110.8, y_max=120.9, confidence=0.5
    )
...
def test_choose_prices_rejects_values_below_min():
    ...
    low_cands = [
        PriceCandidate(
            value=1.0, text="1", score=1.0, box=_mock_box("1"), context=""
        ),
        PriceCandidate(
            value=3.0, text="3", score=0.5, box=_mock_box("3"), context=""
        ),
    ]
...
def test_choose_prices_keeps_valid_price_with_noise():
    ...
    valid = PriceCandidate(
        value=129.99,
        text="129.99",
        score=10.0,
        box=_mock_box("129.99", 0.1),
        context="",
    )
    noise = PriceCandidate(
        value=3.0, text="3", score=0.5, box=_mock_box("3", 0.01), context=""
    )
