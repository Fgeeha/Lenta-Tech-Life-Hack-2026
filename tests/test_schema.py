...
def test_output_columns_no_duplicates():
    assert len(OUTPUT_COLUMNS) == len(set(OUTPUT_COLUMNS)), "Есть дублирующиеся столбцы"


def test_output_columns_order():
    assert OUTPUT_COLUMNS == EXPECTED_COLUMNS, "Порядок столбцов не совпадает с ТЗ §2"
